import { ArrowRight, FileText, ShieldCheck, Upload } from 'lucide-react'
import { Link } from 'react-router-dom'
import { Button } from '@/components/ui/button'
import { PreviewLayout } from '@/components/PreviewLayout'
import { useTranslations } from '@/i18n'

export function HomePage() {
  const dict = useTranslations()
  const highlights = [
    {
      description: dict.home.highlights.localFirst.description,
      icon: Upload,
      title: dict.home.highlights.localFirst.title,
    },
    {
      description: dict.home.highlights.treeEditing.description,
      icon: FileText,
      title: dict.home.highlights.treeEditing.title,
    },
    {
      description: dict.home.highlights.minimalBackend.description,
      icon: ShieldCheck,
      title: dict.home.highlights.minimalBackend.title,
    },
  ]

  return (
    <PreviewLayout title={dict.home.layoutTitle}>
      <div className="px-6 py-10">
        <div className="mx-auto grid max-w-[1400px] gap-6 xl:grid-cols-[1.1fr,0.9fr]">
          <section className="overflow-hidden rounded-[40px] border border-zinc-200/70 bg-white/85 p-8 shadow-sm backdrop-blur-sm lg:p-12">
            <p className="text-xs uppercase tracking-[0.3em] text-zinc-500">{dict.home.headerEyebrow}</p>
            <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-tight text-zinc-950 lg:text-6xl">
              {dict.home.headerTitle}
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-zinc-600">{dict.home.headerDescription}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/builder">
                <Button>
                  {dict.home.openBuilder}
                  <ArrowRight />
                </Button>
              </Link>
              <Link to="/docs">
                <Button variant="outline">{dict.home.readConstraints}</Button>
              </Link>
            </div>
          </section>

          <section className="rounded-[40px] border border-zinc-200/70 bg-zinc-950 p-8 text-zinc-100 shadow-sm lg:p-12">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-300">{dict.home.mvpEyebrow}</p>
            <div className="mt-6 space-y-4 text-sm leading-7 text-zinc-300">
              <p>{dict.home.mvpLineOne}</p>
              <p>{dict.home.mvpLineTwo}</p>
              <p>{dict.home.mvpLineThree}</p>
            </div>
          </section>

          <section className="xl:col-span-2 grid gap-4 md:grid-cols-3">
            {highlights.map((item) => (
              <article
                key={item.title}
                className="rounded-[32px] border border-zinc-200/70 bg-white/80 p-6 shadow-sm backdrop-blur-sm"
              >
                <div className="inline-flex rounded-2xl bg-amber-100 p-3 text-amber-700">
                  <item.icon />
                </div>
                <h2 className="mt-5 text-xl font-semibold text-zinc-950">{item.title}</h2>
                <p className="mt-3 text-sm leading-7 text-zinc-600">{item.description}</p>
              </article>
            ))}
          </section>
        </div>
      </div>
    </PreviewLayout>
  )
}
